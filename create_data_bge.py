...
if __name__ == '__main__':
    ...
    parser.add_argument("--save_pair_path", default="/home/gemai/md1/NAMNT_DA2/data_train_bge/", type=str,help="path to save pair sentence directory")
    ...
    with open(os.path.join(args.save_pair_path, file_name + '.json'), 'w') as output_file:
        for idx, item in tqdm(enumerate(training_items)):
            ...
            for article in relevant_articles:
                ...
                # passage_emb = model.encode(doc_data[concat_id]["title"] + " " + doc_data[concat_id]["text"])
                # scores = util.cos_sim(question_emb, passage_emb)
                # save_dict["pos_scores"].append(scores.item())
            ...
            for idx, idx_pred in enumerate(predictions):
                ...
                if check == 0: # không trùng với ground truth thì cho làm negative
                    ...
                    # passage_emb = model.encode(doc_data[concat_id]["title"] + " " + doc_data[concat_id]["text"])
                    # scores = util.cos_sim(question_emb, passage_emb)
                    # save_dict["neg_scores"].append(scores.item())
