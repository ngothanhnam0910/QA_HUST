...
if __name__ == '__main__':
    ...
    train_path = os.path.join(raw_data, "data_qa.json")
    ...
    for idx, item in tqdm(enumerate(items)):
        ...
        # Lấy ra chỉ số của topk phần tử co score cao nhất (k đang để là 2)
        predictions = np.argpartition(doc_scores, len(doc_scores) - cfg.top_k_bm25)[-cfg.top_k_bm25:]
        # print(f"predictions: {predictions}")
        # print(f"doc_scores[predictions[1]]: {doc_scores[predictions[1]]}")
        # print(f"doc_scores[predictions[0]]: {doc_scores[predictions[0]]}")
        # exit()
        # print(f"shape of prediction :{predictions.shape}")
        # Trick to balance precision and recall
        if doc_scores[predictions[1]] - doc_scores[predictions[0]] >= 2.2:
        ...
        for idx, idx_pred in enumerate(predictions):
            ...
            # Remove prediction with too low score: 10
            if doc_scores[idx_pred] >= 10:
